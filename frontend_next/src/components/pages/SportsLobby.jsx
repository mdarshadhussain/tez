...
export default function SportsLobby() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[calc(100vh-72px)] w-full text-white bg-[#0c0d14] px-6 relative overflow-hidden select-none">
      ...
      <div className="text-center space-y-4 max-w-4xl relative z-10 mb-8">
        ...
        <p className="text-xs md:text-sm text-text-secondary max-w-sm mx-auto font-medium tracking-wide">
      ...
      <div className="relative flex items-center justify-center w-52 h-52 mb-8">
        ...
        <div className="absolute inset-0 animate-[spin_15s_linear_infinite]">
          ...
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#3de796]/10 border border-[#3de796]/30 px-2.5 py-1 rounded-lg text-[8px] font-black text-[#3de796] flex items-center gap-1 shadow-md whitespace-nowrap">
            <span>ODDS</span>
            <span className="text-white font-bold">1.85x</span>
          </div>
        </div>
        ...
        <div className="absolute inset-11 rounded-full border border-double border-white/10 flex items-center justify-center bg-black/40 shadow-inner">
          
          {/* Neon spinner arc */}
          <div className="absolute inset-0 rounded-full border-t-2 border-[#3de796] animate-spin" />
          
          {/* Core soccer ball icon pulsing */}
          <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-[#0f111a] to-[#1e293b] border border-white/10 flex items-center justify-center shadow-2xl relative z-10">
        ...
        <div className="absolute -bottom-1 bg-[#171a25] border border-white/10 px-2.5 py-0.5 rounded-full text-[8px] font-black tracking-wider text-white flex items-center gap-1 shadow-md">
      ...
      <div className="w-full max-w-sm bg-[#11131c]/60 border border-white/5 rounded-2xl p-5 backdrop-blur-xl relative z-10 shadow-2xl">
        {!isWhitelisted ? (
          <form onSubmit={handleWhitelist} className="space-y-3">
            <div className="flex gap-2">
              <input 
                ...
                className="bg-black/30 border border-white/5 hover:border-white/10 focus:border-[#3de796]/30 text-xs rounded-xl px-3.5 py-2.5 w-full text-white outline-none transition-all placeholder:text-[#6b7280] font-medium"
              />
              <button 
                type="submit"
                className="bg-[#3de796] hover:bg-[#5cf2aa] text-[#0f111a] px-4.5 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all hover:scale-102 active:scale-98 cursor-pointer border-0 shrink-0 flex items-center gap-1"
              >
                <span>Join</span>
          ...
          <div className="flex items-center gap-3 py-1 px-1">
            ...
            <div className="min-w-0">
              <div className="text-[10px] font-black text-white uppercase tracking-wider">You're on the Whitelist!</div>
              <p className="text-[9px] text-text-muted truncate mt-0.5">Notifying <code className="text-[#3de796] font-bold">{email}</code> at launch.</p>
            </div>
          </div>
        )}
      </div>
