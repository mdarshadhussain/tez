...
export default function TransactionHistory({ token }) {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case 'bet_place': return <CircleDot size={16} className="text-text-muted" />;
      ...
      default: return <CircleDot size={16} className="text-text-muted" />;
    }
  };
    ...
    <div className="mt-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="bg-transparent border-0 text-accent-cyan hover:text-white transition-colors cursor-pointer flex items-center gap-1 text-xs"
        >
      ...
      {loading ? (
        <div className="text-text-muted text-xs py-4">Loading ledger logs...</div>
      ) : history.length === 0 ? (
        <div className="bg-zinc-950/40 border border-white/5 text-center text-text-muted py-8 rounded-2xl text-xs">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="bg-white/5 p-2 rounded-xl border border-white/5 flex items-center justify-center">
                      {getIcon(tx.type)}
                    </div>
                    <div>
                      <div className="font-bold text-white text-xs">{formatType(tx.type)}</div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="monospace-ledger text-[10px] text-text-dim mt-0.5">
